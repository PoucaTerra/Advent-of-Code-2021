...
out = [len([len(x) for x in row.split("|")[1].strip().split(
    " ") if len(x) in (2, 3, 4, 7)]) for row in ipt]
print(sum(out))
    ...
    for x in decode_this:
        if len(x) == 5:
            ...
            if top_right not in x and bottom_left in x:
                ...
                numbers[0] = x
    summation += int("".join(["".join([str(i) for i, n in enumerate(numbers)
                     if "".join(sorted(n)) == "".join(sorted(x))]) for x in out]))
    numbers = [None] * 10
    top = None
    top_right = None
    bottom_left = None
